...
class LignePol:
    ...
    def tracer(self):
        ...
        for i in range(len(self.__sommets)-1):
            tracer_ligne(self.__sommets[i].x(), self.__sommets[i].y(),
                        self.__sommets[i+1].x(), self.__sommets[i+1].y())
        tracer_ligne(self.__sommets[-1].x(), self.__sommets[-1].y(), self.__sommets[0].x(),
                    self.__sommets[0].y()) # trace la ligne entre le dernier et le premier point


def tracer_ligne(x0, y0, x1, y1):
